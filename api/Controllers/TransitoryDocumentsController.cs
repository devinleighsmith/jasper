...
namespace Scv.Api.Controllers
{
        ...
        ILogger<TransitoryDocumentsController> logger) : ControllerBase
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                var missingFileExamples = string.Join(
                    "; ",
                    missingFiles
                        .Take(5)
                ...
                logger.LogWarning(
                    "Transitory merge rejected due to missing files - MissingCount: {MissingCount}, RequestedFileCount: {RequestedFileCount}, SampleMissingFiles: {SampleMissingFiles}",
                    missingFiles.Count,
                    request.Files?.Length ?? 0,
                    missingFileExamples);
            ...
            {
                var mismatchExamples = string.Join(
                    "; ",
                    sizeMismatches
                        .Take(5)
                        .Select(f =>
                        {
                            var key = CreateTransitoryFileIdentity(f.RelativePath, f.FileName);
                            var matched = searchResultLookup[key];
                            return $"{key} requested={f.SizeBytes} actual={matched.SizeBytes}";
                        }));

                logger.LogWarning(
                    "Transitory merge rejected due to size mismatches - MismatchCount: {MismatchCount}, RequestedFileCount: {RequestedFileCount}, SampleMismatches: {SampleMismatches}",
                    sizeMismatches.Count,
                    request.Files?.Length ?? 0,
                    mismatchExamples);
