...
public class TransitoryDocumentsServiceTests : ServiceTestBase
{
    ...
    [Fact]
    public async Task DownloadFile_ShouldReadFromEfsPath_WhenHeaderIsPresent()
    {
        // Arrange
        var path = "Interior\\Kelowna\\1. January\\8\\010\\Dill, Pickle_R4_Multi_NEL_BailPkg.pdf";
        var fileName = "Dill, Pickle_R4_Multi_NEL_BailPkg.pdf";
        var contentType = "application/pdf";

        var efsFilePath = System.IO.Path.GetTempFileName();
        var expectedBytes = _faker.Random.Bytes(128);
        await System.IO.File.WriteAllBytesAsync(efsFilePath, expectedBytes);

        var jsonEnvelopeStream = new System.IO.MemoryStream(System.Text.Encoding.UTF8.GetBytes("{\"message\":\"File saved to EFS due to size limit\",\"filePath\":\"" + efsFilePath + "\"}"));
        var headers = new Dictionary<string, IEnumerable<string>>
        {
            { "Content-Disposition", [ $"attachment; filename=\"{fileName}\"" ] },
            { "Content-Type", [ contentType ] },
            { "X-EFS-File-Path", [ efsFilePath ] }
        };

        var fileResponse = new FileResponse(200, headers, jsonEnvelopeStream, null, null);
        ...
        using var readBack = new System.IO.MemoryStream();
        await result.Stream.CopyToAsync(readBack);
        Assert.Equal(expectedBytes, readBack.ToArray());

        result.Stream.Dispose();
        Assert.False(System.IO.File.Exists(efsFilePath));
    }
