      ...
      >
        <v-row style="display: flex; flex-wrap: nowrap">
          ...
          <v-col class="px-0" style="overflow: auto">
            <case-header :details="details" :activityClassCd="details.activityClassCd" />
    ...
    setup() {
      ...
      const reloadCaseDetails = () => {
        ...
        getFileDetails();
            ...
            // const storeRouteFile = (routeFileNumber: string) => {
      //   fileNumber.value = routeFileNumber;
      //   if (
      //     !courtFileSearchStore.selectedFiles.some(
      //       (file) =>
      //         file.key === criminalFileStore.criminalFileInformation.fileNumber
      //     )
      //   ) {
      //     console.log('entered');
      //     console.log(criminalFileStore.criminalFileInformation);
      //     courtFileSearchStore.selectedFiles.push({
      //       key: criminalFileStore.criminalFileInformation.fileNumber,
      //       value:
      //         criminalFileStore.criminalFileInformation.detailsData
      //           .fileNumberTxt,
      //     });
      ...
      // };
            // onMounted(async () => {
      //   loading.value = true;
      //   const routeFile = getSingleValue(route.params.fileNumber);
      //   criminalFileStore.criminalFileInformation.fileNumber =
      //     getSingleValue(routeFile);
      //   criminalFileStore.updateCriminalFile(
      //     criminalFileStore.criminalFileInformation
      //   );
      ...
      //   // If case details are being routed to directly, manually set the selected file and add it to store
      //   if (!fileNumber.value) {
      //     storeRouteFile(routeFile);
      //   }
      // });
